use super::VulkanApp;
use super::{math::Vector3SExt, transform::Transform};

use cgmath::ElementWise;
use cgmath::SquareMatrix;
use cgmath::{vec3, Deg, Euler, Matrix4, Vector2, Vector3};
...
impl Camera {
    ...
    pub fn rotate(&mut self, rotation: Vector3<f32>) {
        self.transform
            .rotate(rotation.mul_element_wise(self.rot_speed));
    ...
    pub fn move_relative(&mut self, dir: Vector3<f32>) {
        self.transform
            .move_relative(dir.mul_element_wise(self.move_speed));
    }
    ...
    fn default() -> Self {
        Camera {
            transform: Transform::default()
                .set_position(vec3(0.0, 1.0, 2.0))
                .set_rotation(vec3(0.0, 0.0, 0.0)),
            max_pitch: 85.0,
