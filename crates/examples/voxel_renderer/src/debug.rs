...
impl MovementDebug {
    ...
    pub fn read(&mut self, camera: &mut Camera, index: usize) -> Result<()> {
        ...
        let pos = Vec3::new(f32::from_str(parts[0])?, f32::from_str(parts[1])?, f32::from_str(parts[2])?);
        let dir = Vec3::new(f32::from_str(parts[3])?, f32::from_str(parts[4])?, f32::from_str(parts[5])?);
