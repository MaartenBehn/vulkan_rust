use app::glam::{Vec2, UVec2, IVec2};
use crate::{aabb::AABB, particle::{Particle, PARTICLE_RADIUS}};
...
impl Chunk {
    ...
    pub fn add_particle(
        ...
        if part.is_none() {
            ...
            part = self.parts.last_mut().unwrap());
        }
...
pub fn hex_to_chunk_part_pos(hex: UVec2) -> UVec2 {
    UVec2::new(
        hex.x / CHUNK_PART_SIZE, 
        hex.y / CHUNK_PART_SIZE)
}
