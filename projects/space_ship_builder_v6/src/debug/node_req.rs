...
impl DebugController {
    pub fn update_node_req(
        ...
        let node_id_bits = self.get_node_req_node_id_bits(
            self.node_req_renderer.mesh.size,
            self.node_req_renderer.mesh.render_size,
            rules,
            self.node_req_renderer.rule_index,
        );

        self.node_req_renderer.update_renderer(
            &node_id_bits,
            image_index,
            context,
            descriptor_layout,
    ...
    fn get_node_req_node_id_bits(
        ...
        let node_reqs = &rules.node_rules[rule_index];
        for x in 0..rules_size.x {
            for y in 0..rules_size.y {
                for z in 0..rules_size.z {
                    ...
                    let r = node_reqs.iter().find(|(pos, _)| *pos == test_pos);
                    if r.is_none() {
                        continue;
                    }

                    let mut pattern_counter = 0;
                    let (_, possible_nodes) = r.unwrap();
