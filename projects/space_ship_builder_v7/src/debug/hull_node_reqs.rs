...
impl DebugHullNodeReqRenderer {
    ...
    fn update_rule_index(
        ...
        if controls.t && (self.last_action_time + NEXT_NODE_SPEED) < total_time {
            ...
            self.rule_index += 1;
            if self.rule_index >= hull_solver.multi_node_reqs.len() {
                self.rule_index = 0;
            }
...
impl DebugController {
    pub fn update_hull_node_req(
        ...
        self.add_cube(
            Vec3::ONE * (HULL_SIZE / 2) as f32,
            Vec3::ONE * ((HULL_SIZE / 2) + 1) as f32,
    ...
    fn get_node_req_node_id_bits(
        ...
        node_debug_node_id_bits[middle_index] = hull_solver.multi_node_reqs[rule_index].0.into();

        let reqs = &hull_solver.multi_node_reqs[rule_index].1;
        for x in 0..rules_size.x {
            for y in 0..rules_size.y {
                for z in 0..rules_size.z {
                    let node_pos = ivec3(x, y, z);
                    let test_offset = node_pos - middle_pos;

                    let index = reqs.iter().position(|(offset, _)| *offset == test_offset);
                    if index.is_none() {
                        continue;
                    }

                    let mut pattern_counter = 0;
                    let possible_nodes = &reqs[index.unwrap()].1;
                        ...
                        for iy in 0..pattern_block_size.y {
                            for ix in 0..pattern_block_size.z {
                                ...
                                if node_id.is_any() {
                                    let one_cell_size = Vec3::ONE / pattern_block_size.as_vec3();
                                    let p = pattern_pos.as_vec3() * one_cell_size;
                                    self.add_cube(p, p + one_cell_size, vec4(0.0, 1.0, 1.0, 1.0));
                                }
