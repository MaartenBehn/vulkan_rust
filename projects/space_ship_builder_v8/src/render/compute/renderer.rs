...
impl ComputeRenderer {
    pub fn new() -> Result<ComputeRenderer> {
        Ok(ComputeRenderer {
...
impl RenderFunctions for ComputeRenderer {
    ...
    fn render(&self, buffer: &CommandBuffer, image_index: usize, render_object: &RenderObject) -> Result<()> {
        Ok(())
    }

    fn on_rules_changed(&mut self, rules: &Rules, context: &Context, num_frames: usize) -> Result<()> {
        Ok(())
    }
}